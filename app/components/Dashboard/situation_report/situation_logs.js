...
export default class SituationLogs extends Component {
  ...
  displaySituationReportPerDay() {
    ...
    Sync.clientToServer("SituationReportLogs").then(() => {
      setTimeout(() => {
        fetch('http://192.168.1.101:5000/api/situation_report/get_all_situation_report').then((response) => response.json())
          .then((responseJson) => {
            let to_local_data = [];
    ...
    if (role_id == 1) {
      ...
      Alert.alert(
        ...
        [
          ...
          {
            text: 'Yes', onPress: () => {
              fetch('http://192.168.1.101:5000/api/situation_report/delete_situation_report', {
                method: 'POST',
                headers: {
  ...
  selectDateToAddReport(date) {
    ...
    if (compare_date == false) {
      ...
      fetch('http://192.168.1.101:5000/api/situation_report/get_report_by_date', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
