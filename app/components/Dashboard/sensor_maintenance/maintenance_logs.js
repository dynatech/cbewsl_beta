...
export default class MaintenanceLogs extends Component {
  ...
  displayMaintenanceLogsPerDay() {
    ...
    Sync.clientToServer("SensorMaintenanceLogs").then(() => {
      setTimeout(() => {
        fetch('http://192.168.8.100:5000/api/sensor_maintenance/get_all_sensor_maintenance').then((response) => response.json())
          .then((responseJson) => {
            let to_local_data = [];
  ...
  selectDateToAddLogs(date) {
    ...
    fetch('http://192.168.8.100:5000/api/sensor_maintenance/get_report_by_date', {
      method: 'POST',
      headers: {
