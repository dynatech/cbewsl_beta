...
export default class ModifyHazardData extends Component {
  ...
  removeLog(id) {
    ...
    fetch('http://192.168.8.100:5000/api/hazard_data/delete_hazard_data', {
      method: 'POST',
      headers: {
  ...
  getAllHazardData() {
    Notification.endOfValidity();
    fetch('http://192.168.8.100:5000/api/hazard_data/get_all_hazard_data').then((response) => response.json())
      .then((responseJson) => {
        let to_local_data = [];
        let counter = 0
        let hazard_data = [];
