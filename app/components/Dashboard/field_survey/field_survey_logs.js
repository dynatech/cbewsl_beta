...
export default class FieldSurveyLogs extends Component {
  ...
  removeLog(id) {
    ...
    fetch('http://192.168.1.101:5000/api/field_survey/delete_field_survey', {
      method: 'POST',
      headers: {
  ...
  getAllFieldSurveyLogs() {
    ...
    Sync.clientToServer("FieldSurveyLogs").then(() => {
      setTimeout(() => {
        fetch('http://192.168.1.101:5000/api/field_survey/get_all_field_survey').then((response) => response.json())
          .then((responseJson) => {
            let field_logs = [];
            let to_local_data = [];
            let counter = 0
