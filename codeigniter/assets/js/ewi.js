...
function getCandidateAndLatestAlerts() {
    ...
    // }).done(() => {
        ...
        $.ajax({
            url: "http://192.168.1.101:5000/api/monitoring/get_candidate_and_current_alerts",
            beforeSend: function (xhr) {
                xhr.overrideMimeType("text/plain; charset=x-user-defined");
            }
        }).done(function (data) {
            ...
            if (json_data.leo.routine.length > 0) {
                displayRoutine(json_data.leo.routine, candidate_alerts, true, json_data)
            }
...
function updateAlertGen () {
    return fetch("http://192.168.1.101:5000/api/monitoring/update_alert_gen");
}
...
function alertValidation(trigger_id, valid, user_id, candidate_alerts, alert_data, trigger_type) {
    ...
    $("#save_alert_validation").click(function () {
        ...
        let url = "http://192.168.1.101:5000/api/monitoring/update_alert_status"
...
function publicAlert(is_onset = false) {
    let url = 'http://192.168.1.101:5000/api/monitoring/update_alert_gen/' + is_onset;
...
function getLatestMomsPerInstance(instance_ids) {
    let url = "http://192.168.1.101:5000/api/manifestations_of_movement/get_latest_site_moms_alerts"
    fetch(url, {
        method: 'POST',
        dataType: 'jsonp',
        headers: {
...
function sendEwiToEmail() {
    ...
    $("#confirm_send_ewi").click(function () {
        ...
        let url = "http://192.168.1.101:5000/api/ewi/send_ewi_via_email";
        let data = {
            email: email,
            html: html
        }
